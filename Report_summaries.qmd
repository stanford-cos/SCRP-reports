...
comp_vessels <- read_csv(file.path(data_dir,"CompiledVesselsIndicators-2023-02-21.csv"))
...
risky_vessels <- comp_vessels %>% 
  ...
  select(c_name, c, c_listname, c_imo, c_mmsi, c_flag, 70:length(comp_vessels))
...
report_generally(nesi_srilanka)

check_kde_list(nesi_srilanka)

assess_risk(nesi_srilanka)

create_context(nesi_srilanka, itf_foc)
