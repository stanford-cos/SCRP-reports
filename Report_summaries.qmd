...
dum_vessels <- read_excel(file.path(data_dir, "pvr_vessels_subset_2022-02-18.xlsx"), sheet = 2) # replace vessel names with non-identifying values
dum_n <- 1:nrow(dum_vessels)
for (i in dum_n) {
  dum_vessels$vessel_record_id[i] <- paste("vessel ", i)
}
...
give_context <- function(supply_chain, foc_list){
  ...
  summarise("company" = unique(c),
            ...
            "landing_country" = paste(unique(c_country), sep = ";"),
            "cpi" = NA,
            ...
            "species_harvest" = paste(unique(c_species), sep = ";"),
...
# replace supply_chain arguement with one of interest to generate report values

speak_generally(nesi_maldives)

check_kde_list(nesi_maldives)

assess_risk(nesi_maldives)

give_context(nesi_maldives, itf_foc)

detail_vessels(nesi_maldives)
