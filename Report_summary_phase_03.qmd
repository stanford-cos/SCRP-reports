...
question_3_df <- survey_analysis_df %>%
  select(Q3:Q5) %>% 
...
question_3_fig_df <- question_3_df %>% 
  count(response) %>% 
  arrange(desc(n))


fig_3 <- mk_yn_fig(question_3_fig_df)
fig_3
...
```{r echo=FALSE, include= T, results="asis"}
# display text open text responses
for (x in seq_along(question_3_df$response)) {
  if(question_3_df$response[x] == "Yes"){
    formatted_string <- paste("- ", question_3_df$value[x])
...
# display text open text responses
for (x in seq_along(question_3_df$response)) {
  if(question_3_df$response[x] == "Sometimes"){
    formatted_string <- paste("- ", question_3_df$value[x])
    cat(formatted_string, "\n")
  }
}
...
# display text open text responses
for (x in seq_along(question_3_df$response)) {
  if(question_3_df$response[x] == "No"){
    formatted_string <- paste("- ", question_3_df$value[x])
    cat(formatted_string, "\n")
  }
}
