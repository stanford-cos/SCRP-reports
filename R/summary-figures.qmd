...
df_left <- data.frame(c_flag = c("a;b;c", "d;e;f", "g;h;i"))
df_right <- data.frame(c_flag = c("b", "e", "h"), value = c(1, 2, 3))
df_join <- df_left %>%
  filter(str_detect(c_flag, paste(df_right$c_flag, collapse = "|"))) #%>%
  #left_join(df_right, by = "c_flag")
