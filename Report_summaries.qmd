...
all_vessels <- read_csv(file.path(data_dir,"CompiledVesselsIndicators-2023-02-21.csv"))
...
assess_risk <- function(supply_chain){
  ...
  summarize("company" = unique(c),
      ...
      # High Risk Indicators      
            "iuu_listed" = sum(as.numeric(vv_iuu_listed)),
...
detail_vessels <- function(supply_chain) {
  detail <- supply_chain %>%
    ...
    mutate(
      ...
      "long_trip_logic" = map_long_trip_n > 0,
  ...
  #  select(c_name, iuu_logic:flag_changes)
...
speak_generally(nesi_srilanka)

check_kde_list(nesi_srilanka)

assess_risk(nesi_srilanka)

give_context(nesi_srilanka, itf_foc)

detail_vessels(nesi_srilanka)
