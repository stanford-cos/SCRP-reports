...
bb_tuna <- read_csv(file.path(data_dir,"vessel_indicators_vv_marks_2023-02-10.csv"))

summary_general <- bb_tuna %>% 
...
report_indicators <- bb_tuna %>% 
  summarize("iuu_listed" = sum((vv_iuu_listed)),
            "rfmo_no_auth_events" = sum(vv_rfmo_events),
            "rfmo_no_auth_vessels_n" = sum(vv_rfmo_events > 0),
            ...
            "vessel_name_change_n" = sum(vv_name_change),
            "vesse;_flag_change_n" = sum(vv_flag_change)
  )
